...
# Parse arguments
MODE=2           # pretrained or continual
...
case $MODE in
  ...
  continual)                                                                  # Evaluate all continual models on all tasks
    ...
    [[ ${#SELECTED_TASKS[@]} -eq 0 ]] && SELECTED_TASKS=(all)
    ;;
  [1-9])                                                                      # Evaluate specific task
    [[ ${#SELECTED_MODELS[@]} -eq 0 ]] && SELECTED_MODELS=($((MODE-1)))
    [[ ${#SELECTED_TASKS[@]} -eq 0 ]] && SELECTED_TASKS=($((MODE-1)))
    ;;
