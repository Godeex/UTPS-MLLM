...
class DMoLELinear(nn.Module, LoraLayer):
    """
    D-MoLE线性层：支持动态专家路由的LoRA线性层
    对应论文中的公式(2)：f^t_l(x) = W^0_l x + Σ_{k=1}^{t-1} I^k_l · g^k_l(x) · ΔW^k_l x + I^t_l · ΔW^t_l x

    继承自nn.Module和LoraLayer，结合了标准线性层和LoRA功能
    """
    def __init__(
        self,
        base_layer,
        ...
        **kwargs,
        ...
        super().__init__()
        LoraLayer.__init__(self, base_layer, **kwargs)  # 初始化LoRA层基类

        self.fan_in_fan_out = fan_in_fan_out
        self._active_adapter = adapter_name     # 当前激活的适配器

        # ==================== MoLE特有参数 ====================
        self.num_experts = kwargs.pop("num_experts", 8)
        self.expert_ids = kwargs.pop("expert_ids", None)
        # 专家掩码：控制哪些专家被激活（对应I^k_l指示器）
    ...
    def update_layer(
        ...
        """
        更新或初始化LoRA层参数
        对应LoRA参数初始化：ΔW^i_l = B^i_l A^i_l
        """
        if r <= 0:
            raise ValueError(
        ...
        # ==================== 关键：创建多个专家 ====================
        # 对应多个LoRA专家：每个专家对应一个任务
        # lora_A: 降维矩阵 (in_features × r) × num_experts
        # lora_B: 升维矩阵 (r × out_features) × num_experts
        update_lora_module(
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        for adapter_name in self.active_adapters:
            ...
            # ==================== 关键：选择激活的专家 ====================
            # 专家选择：根据expert_masks选择哪些专家参与计算
            # 在训练时：只选择当前任务和相关的旧任务专家
            # 在推理时：根据自动编码器路由选择专家
