...
class MoELoraModel(LoraModel):
    ...
    def _create_and_replace(
        ...
        if lora_config.dmole_arch and not any(
            ...
            # )
            rank0_print(
                f"Skipping {current_key} because it is not in arch"
            )
            return
        ...
        if lora_config.dmole_arch and len(lora_config.dmole_arch[current_key]) == 0:
            ...
            # )
            rank0_print(
                f"Skipping {current_key} because it is empty in arch"
            )
            return
        ...
        rank0_print(f"Updating {current_key} because it is in arch")
    ...
    def set_expert_masks(self, task_ids):
        """
        设置专家掩码：控制哪些LoRA专家被激活
        对应专家选择机制，实现任务特定的知识激活

        参数：
            task_ids: 要激活的任务ID列表（或单个任务ID）
        """
        if not isinstance(task_ids, list):
            task_ids = [task_ids]
        for _, layer in self.named_modules():
            if isinstance(layer, DMoLELinear):
                # 关键逻辑：如果专家对应的任务ID在task_ids中，则激活
