...
def main():
    ...
    # 1. 准备阶段：设置 Hook 捕捉激活值
    activations = {}
    def get_activation_hook(name):
        def hook(model, input, output):
            # 捕捉前向传播中激活值的量级 (取平均以降低 batch 噪声)
            # 使用 detach() 避免增加计算图负担
            if name not in activations:
                activations[name] = output.detach().abs().mean().item()
            else:
                activations[name] = (activations[name] + output.detach().abs().mean().item()) / 2
        return hook
    ...
    logger.info("Computing Universal Taylor Proxy Score (|g * w| * |a|)...")
    ...
    if dist.is_initialized():
        ...
        # B. 同步激活值：将所有显卡上的激活量级取平均，消除数据分布偏差
        for name in activations:
            act_tensor = torch.tensor([activations[name]], device=model.device)
            dist.all_reduce(act_tensor, op=dist.ReduceOp.AVG)
            activations[name] = act_tensor.item()
    ...
    if rank == 0:
        zc_proxy_score_dict = defaultdict(float)

        for name, module in model.named_modules():
            if any(name.endswith(target) for target in target_modules):
                # A. 计算该层所有参与 LoRA 的总参数量 (numel)
                # 只统计有梯度的参数，确保计算的是可训练权重的密度
                layer_param_count = sum(p.numel() for p in module.parameters() if p.requires_grad)
                ...
                for param in module.parameters():
                    if param.grad is not None:
                        # # 分母平滑：多 batch 累积均摊
                        # avg_grad = param.grad / max_batch_num
                        # 计算 Taylor-W: |g * w|
                        param_influence += (param.grad * param.detach()).norm().item()

                # B. 引入激活因子
                act_factor = activations.get(name, 1.0)

                # C. 核心优化：引入“参数密度”评分
                # 公式：(总敏感度 * 激活贡献) / 总参数量
                # 这样得到的是该层“平均每个参数”的重要性
                raw_score = (param_influence * act_factor * world_size)
                density_score = raw_score / layer_param_count * 1000

                zc_proxy_score_dict[name] = density_score

        # 4. 排序：此时选出的是单位效率最高的层
        zc_proxy_score_list = sorted(
            zc_proxy_score_dict.items(), key=lambda x: x[1], reverse=True
        )

        logger.info(f"Successfully computed scores for {len(zc_proxy_score_list)} layers.")

        os.makedirs(os.path.dirname(model_args.zc_proxy_score_save_path), exist_ok=True)
        df = DataFrame(zc_proxy_score_list, columns=["layer", "score"])
        df.to_csv(model_args.zc_proxy_score_save_path, index=False)

        logger.info(f"Universal scores saved to {model_args.zc_proxy_score_save_path}")
